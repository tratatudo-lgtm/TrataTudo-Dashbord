...
/**
 * Mantemos o nome "searxngSearch" para não mexer no route.ts.
 * Chama Whoogle Proxy:
 *   GET {WEB_PROXY_URL}/search?q=...&n=...&domains=...
 *   Header: X-TrataTudo-Key: WEB_PROXY_KEY
 *
 * Env:
 *   WEB_PROXY_URL=http://...
 *   WEB_PROXY_KEY=...
 *   WEB_PROXY_DOMAINS_DEFAULT=jf-vcca.pt,cm-valenca.pt   (opcional)
 */
export async function searxngSearch(query: string, n = 5): Promise<WebSource[]> {
  ...
  const domainsDefault = safeStr(process.env.WEB_PROXY_DOMAINS_DEFAULT);
  ...
  // 🔒 filtro por domínios (sites oficiais)
  if (domainsDefault) {
    url.searchParams.set("domains", domainsDefault);
  }
