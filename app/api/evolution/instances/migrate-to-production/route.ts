...
export async function POST(request: Request) {
  try {
    ...
    // 1) garante instância na Evolution
    ...
    // 2) desativar instâncias antigas no Supabase
    await supabase
    ...
    // 3) ativar a instância client-<id>
    ...
    return NextResponse.json({
      ...
      integration,
      migrated: true,
      previous_status: clientRow.status,
      new_status: "active",
    });
  } catch (error: any) {
    console.error("Migrate to production error:", error);
